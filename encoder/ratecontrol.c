...
static NOINLINE uint32_t x264_ac_energy_mb( x264_t *h, int mb_x, int mb_y, x264_frame_t *frame, uint32_t *energy )
{
    ...
    if( h->mb.b_adaptive_mbaff )
    {
        ...
        uint32_t var_interlaced_y   = ac_energy_plane( h, mb_x, mb_y, frame, 0, 1, 1 );
        uint32_t var_interlaced_uv  = ac_energy_plane( h, mb_x, mb_y, frame, 1, 1, 1 );
        uint32_t var_progressive_y  = ac_energy_plane( h, mb_x, mb_y, frame, 0, 0, 0 );
        uint32_t var_progressive_uv = ac_energy_plane( h, mb_x, mb_y, frame, 1, 0, 0 );
    ...
    {
        energy[0] = ac_energy_plane( h, mb_x, mb_y, frame, 0, PARAM_INTERLACED, 1 );
        energy[1] = ac_energy_plane( h, mb_x, mb_y, frame, 1, PARAM_INTERLACED, 1 );
...
static NOINLINE void get_image_mb( x264_t *h, int mb_x, int mb_y, x264_frame_t *frame, int field, int *luma, int *bluePoint )
{
...
static NOINLINE float x264_adjust_OreAQ( x264_t *h, int mb_x, int mb_y, x264_frame_t *frame, uint32_t *energy )
{
    ...
    if( h->param.rc.b_aq_debug )
        x264_write_aqlog_body( h, mb_x + mb_y*h->mb.i_mb_stride, frame, luma, bluePoint, (float)_energy_y, (float)_energy_uv );
...
parse_error:
    ...
    if( h->param.rc.b_aq_debug )
    {
        rc->p_aq_debug = fopen( h->param.rc.psz_aq_debug_name, "wb" );
        if( rc->p_aq_debug == NULL )
        {
            x264_log(h, X264_LOG_ERROR, "ratecontrol_init: can't open AQ debug file\n");
            return -1;
        }
        setvbuf( rc->p_aq_debug, NULL, _IOFBF, 1024*1024*8 ); // 8M Buffer
        x264_write_aqlog_header( h, "OreAQ05", 0,
           h->param.rc.f_aq_strengths[0], h->param.rc.f_aq_strengths[1], h->param.rc.f_aq_sensitivity, 0,
           h->param.rc.i_aq_boundary );
    }
